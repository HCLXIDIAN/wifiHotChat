...
import com.ty.hcl.listener.inter.OnUDPReceiveMessage;
import com.ty.hcl.model.UDPMessage;
import com.ty.hcl.model.User;
import com.ty.hcl.ui.Set;
...
public class UDPMessageListener extends UDPListener{
	...
	private OnUDPReceiveMessage onReceiveMessage;//在chatService里接收数据
	//保存当前在线用户，键值为用户的ip
    ...
    try {
	    ...
	    switch (type) {
				case ADD_USER://增加一个用户
					...
					//构造回送报文内容
					if(!ChatApplication.mainInstance.getLocalIp().equals(user.getIp())){
						users.put(sourceIp, user);
						send(ChatApplication.mainInstance.getMyUdpMessage("", LOGIN_SUCC).toString(), packet.getAddress());
					}
					break;
				...
				case HEART_BEAT://心跳包检测
					send(ChatApplication.mainInstance.getMyUdpMessage("", HEART_BEAT_REPLY).toString(), packet.getAddress());//回复心跳包
				...
				case REQUIRE_ICON://请求头像
					File file=new File(ChatApplication.iconPath+Set.iconName);
					if(file.exists()){
						...
						listener.sendFile(sourceIp, file, ChatApplication.iconPath, ChatApplication.mainInstance.getDeviceCode());
					}
					break;
			}
	    if(onReceiveMessage!=null)
	    	onReceiveMessage.onReceive(type);//在onReceive里面通知刷新数据
  ...
  void noticeOffline() {
		try {
			send(ChatApplication.mainInstance.getMyUdpMessage("", REMOVE_USER).toString(), InetAddress.getByName(Constant.ALL_ADDRESS));
	...
	public void noticeOnline(){
		try {
			send(ChatApplication.mainInstance.getMyUdpMessage("", ADD_USER).toString(), InetAddress.getByName(Constant.ALL_ADDRESS));
		} catch (UnknownHostException e) {
			e.printStackTrace();
		}
	}
